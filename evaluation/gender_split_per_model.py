...
def _plot_model(
    ...
    for yp, (cat, var) in zip(y_positions, ordered_vars):
        for vals, color, offset in [
            ...
            # whisker for std across scenarios
            if std_val > 0:
                ax.plot([mean_val - std_val, mean_val + std_val], [yp + offset, yp + offset],
                        color="#666666", linewidth=0.9, zorder=4)
    ...
    ax.set_xlabel("Mean Δ (female / male)", fontsize=9.5)
    ax.set_title(f"{MODEL_DISPLAY.get(model_name, model_name)} gender split by variation", fontsize=11.2, fontweight="bold")
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument("--evaluation-root", default="output/evaluation")
    ...
    per_model = {d.name: _load_model_data(d) for d in model_dirs}
    ordered_vars = _build_ordered_variations(per_model)
    ...
    for model_dir in model_dirs:
        _plot_model(
            ...
            ordered_vars,
            evaluation_root / "eval_charts" / f"{model_dir.name}_gender_split.png",
        )
