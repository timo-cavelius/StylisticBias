...
def _plot(
    ...
    ax.set_xlabel("Mean Δ (averaged across all models and scenarios)", fontsize=10)

    ax.text(
        0, 1.06, "Gender Asymmetry — Mean Δ averaged across all models",
        transform=ax.transAxes, fontsize=13, fontweight="bold",
        va="bottom", clip_on=False,
    )
    ax.text(
        0, 1.02,
        "f − m shown next to each bar pair  ·  positive = female higher  ·  negative = male higher",
    ...
    ax.legend(handles=[female_patch, male_patch], loc="lower right",
...
def main():
    ...
    all_model_data = {d.name: _load_model_data(d) for d in model_dirs}
    avg_data       = _average_across_models(all_model_data)
    ordered_vars   = _build_ordered_variations(avg_data)

    # Keep only variations present in both genders
    both = avg_data.get("female", {}).keys() & avg_data.get("male", {}).keys()
    ordered_vars = [(cat, var) for cat, var in ordered_vars if var in both]
    print(f"Total variations (both genders): {len(ordered_vars)}")
