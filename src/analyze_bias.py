...
Reads `scenario_choices.csv` and `variation_choices.csv` from an evaluation folder
and computes a simple bias strength metric: for each scenario, bias = |top_proportion - 0.5|.

Outputs:
- scenario_biases.csv  (scenario, top_option, top_count, total, top_proportion, bias)
- variation_biases.csv (variation, mean_bias, median_bias, min_bias, max_bias, scenarios_count)
- bias_summary.txt     (human-readable weakest N lists)

Usage:
  python3 src/analyze_bias.py --evaluation-dir output/evaluation/llave_next --top-n 20
"""
...
def analyze_scenarios(scenario_rows):
    # aggregate counts per scenario -> option
    ...
    for r in scenario_rows:
        ...
        try:
            ...
            # fall back to proportion * total if provided
            try:
    ...
    # sort by bias ascending (weakest first)
    out_rows.sort(key=lambda r: float(r["bias"]))
    return out_rows


def analyze_variations(variation_rows):
    # for each variation and scenario compute bias, then summarize per variation
    ...
    # sort by mean_bias ascending (weakest overall first)
