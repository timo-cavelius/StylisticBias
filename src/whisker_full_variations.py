...
Improvements over the original:
  - Much thicker whisker lines, larger caps and markers → clearly readable
  - Wider per-model jitter so 6 models don't crowd each other
  - Larger figure and fonts throughout
  - Consistent model colours with the other cl_* scripts
  - Cleaner category shading and separators

Output: output/evaluation/eval_charts/whisker_full_variations.png

Usage:
  python3 src/whisker_full_variations.py
"""
...
# ---- Sizes / weights — change these to tune readability globally ----
MARKER_SIZE       = 9.5    # dot diameter
MARKER_EDGE_W     = 1.4    # white halo around dot
WHISKER_LINEWIDTH = 3.2    # vertical error-bar line thickness
CAP_SIZE          = 6.0    # horizontal cap half-width in points
CAP_THICK         = 2.8    # cap line thickness
JITTER_SPAN       = 0.44   # total width occupied by all models per x-position
...
SIGNED_CMAP = LinearSegmentedColormap.from_list(
    "signed_bg", ["#F1C9C9", "#FCFCFD", "#CDEBD3"]
)


# ---------------------------------------------------------------------------
# Data helpers  (identical logic to original, no behaviour change)
# ---------------------------------------------------------------------------
...
def plot(summary, models, variations, output_path: Path) -> None:
    ...
    for panel_idx, gender in enumerate(GENDERS):
        ...
        # ---- gradient background (red→white→green on x-axis) ----
        ax.imshow(
        ...
        # ---- gender-specific section divider ----
        n_shared = len(shared)
