#!/usr/bin/env python3
"""Generate a LaTeX table for the strict Table 2 variant.

This reads category_variation_strength_independent.csv for each model and
writes a copy of the paper-style table using face-level permutation p-values.
"""
...
CSV_OUT_PATH = OUT_DIR / "table2_final_independent.csv"
...
def load_data():
    ...
    for model in MODEL_ORDER:
        csv_path = EVAL_DIR / model / "category_variation_strength_independent.csv"
...
def build_latex(data):
    all_vals = [data[m].get(c, {}).get("strength") for m in MODEL_ORDER for c in CAT_ORDER]
    ...
    lines.append(
        ...
        r"Significance is computed with a face-level label permutation test, BH-corrected within model.}"
    )
    ...
    for model in MODEL_ORDER:
        ...
        for cat in CAT_ORDER:
            ...
            val_cells.append(bg + f"${_fmt(s)}$" + _sig_marker(d.get("p_bh")))
            ci_cells.append(bg + _fmt_ci(lo, hi))

        lines.append(rf"\modelicon{{{icon}}}~{{{label}}}" + " & " + " & ".join(val_cells) + r" \\")
    ...
    lines.append(r"\label{tab:base_variation_independent}")
    ...
    for model in MODEL_ORDER:
        ...
        for cat in CAT_ORDER:
            ...
            if s is None:
                ...
                row.append(f"{s:.3f}{sig} [{_fmt(lo)}, {_fmt(hi)}]")
        rows.append(row)
    return rows
...
def main():
    ...
    out_path = OUT_DIR / "variation_strength_table_independent.tex"
    out_path.write_text(latex)
    print(f"Saved to {out_path}\n")
    print(latex)

    csv_rows = build_csv(data)
    with CSV_OUT_PATH.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["model", "age", "body_type", "ethnicity", "gender"])
