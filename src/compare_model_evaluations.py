...
def _load_base_face_category_variation_strength(csv_path: Path) -> dict[str, float]:
    """Load category variation strength from base-face analysis.
    
    This file is generated by evaluate_base_faces.py and contains metrics showing
    how much demographic categories (age, gender, ethnicity, body_type) affect
    model judgments.
    """
    strengths: dict[str, float] = {}
...
def _plot_base_face_category_variation(models: list[dict], output_path: Path):
    ...
    fig, ax = plt.subplots(figsize=(10, 5))
    for i, model in enumerate(models):
        vals = [model["base_face_category_variation"].get(cat, 0.0) for cat in categories]
        offset = (i - (len(models) - 1) / 2) * width
        ax.bar(x + offset, vals, width=width, label=model["name"])

    ax.set_title("Base-Face Category Variation Strength\n(How much judgments differ across demographic groups)")
    ax.set_ylabel("variation strength (std of means across groups)")
    ax.set_xticks(x)
...
def _write_outputs(models: list[dict], output_dir: Path):
    ...
    if base_face_categories:
        ...
        for cat in base_face_categories:
            row = [cat]
            for m in models:
                row.append(m["base_face_category_variation"].get(cat, ""))
            base_face_var_rows.append(row)
        
        base_face_var_header = ["category_type"] + [m["name"] for m in models]
        _write_csv(
            output_dir / "base_face_category_variation_strength.csv",
            base_face_var_header,
            base_face_var_rows,
        )
    ...
    _plot_radar_comparison(
        models,
        base_radar_categories,
        "polarization_by_category",
        "Base-Face Category Bias Strength (Radar)",
        plots_dir / "base_face_category_radar.png",
        "abs(mean(p(option_a)) - 0.5)",
    )
    _plot_radar_comparison(
        models,
        base_radar_categories,
        "polarization_by_category",
        "Base-Face Category Bias Strength (Radar, Raw)",
        plots_dir / "base_face_category_radar_raw.png",
        "abs(mean(p(option_a)) - 0.5)",
    ...
    if base_face_categories_sorted:
        for cat in base_face_categories_sorted:
            ...
            for m in models:
                strength = m["base_face_category_variation"].get(cat)
                if strength is not None:
                    summary_lines.append(f"  {m['name']}: {strength:.6f}")
    summary_lines.append("")
    ...
    summary_lines.append("base_face_category_radar.png compares age, gender, ethnicity, and body_type.")
    summary_lines.append("variation_category_radar.png compares category-level variation strengths across variation types.")
    summary_lines.append("base_face_category_radar_raw.png uses raw (unnormalized) values.")
    ...
    md_lines.extend(
        [
            ...
            "- `plots/base_face_category_radar.png`: radar chart for base-face bias strength",
            "- `plots/base_face_category_radar_raw.png`: raw-value radar chart for base-face bias strength",
